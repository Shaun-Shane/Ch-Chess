...
unsigned int Zobrist::getCurKey()
{
    return this->curZobristKey;
}
unsigned int Zobrist::getCurLock()
{
    return this->curZobristLock;
}
...
Zobrist::Zobrist(int squares[], int sidePlayer, bool mirror)
{
    ...
    if (sidePlayer == 1)
        this->zobristUpdateChangeSide();
}
int Zobrist::getMoveFromLib(int squares[], int sidePlayer, const Library lib[])
{
    bool mirrorFlag = 0;
    ...
    if (id == -1)
    {
        ...
        id = binarySearchLib(lib, zoTemp.curZobristLock >> 1, START_LIBRARY_SIZE);
    ...
    if (id == -1)
        return 0;
    //找出所有该局面下可能的走法
    while (id >= 0 && lib[id].situation == lockValue)
        id--;
    id++;
    ...
    while (id < START_LIBRARY_SIZE && lib[id].situation == lockValue)
    {
        ...
        libTemp.moveInfo = mirrorFlag ? mirrorMov(lib[id].moveInfo) : lib[id].moveInfo;
